...
```{r fig.show="hide"}
hist(yearlyABperSB$SBperAB) ## determine destribution of SBperAB, not important to the answer
```
...
# 4 A baseball player is said to be continuously playing if he's playing for *consecutive years. Given the years that some baseball player played in, write the function activeYears which computes the sequence of the lengths of the continuous playing. 
```{r}
...
```
### checking to see what it should be... looks like 1 5 2 1 for aardsda01
```{r results="hide"}
df1[df1$playerID=="aardsda01", "yearID"]
```
